...
def core_sixgod(surl, params, devices, data:dict={},  common=None, header=None, lanusk="", log=False):
    ...
    if devices:
        # x-tt-dt 是服务端 device_register 下发的 device_token，不是本地算的签名。
        # 空值不要带头，否则业务接口会按无效设备身份拒绝（111104）。
        token = str(devices.get("x_tt_dt") or "").strip()
        if token:
            headers["x-tt-dt"] = token
        ua = str(devices.get("ua") or "").strip()
        if ua:
