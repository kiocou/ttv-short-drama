...
3. **完整功能模块**：
   ...
   - **播放器核心工作区 (Player Workspace)**：
     ...
     - 画质增强入口：如实报告当前仅有「原生」一档——真实补帧 SDK（小黄鸭 / RIFE）尚未接入，界面不会把它显示成可用。
     - 实时全链路诊断面板（SessionID、实时帧率、已解码帧数、丢帧数与占比、画面尺寸、片源类型、缓冲余量），全部取自 WebView 实测值。
     - 快捷键支持（空格播放暂停、←/→ 快进退、↑/↓ 音量、F 全屏、[/] 换集）。
     - 播放加速：打开详情页即预签名接下来几集的播放直链；整集解析由 ffmpeg 直连完成（拉流 + 解密 + 转存一次做完）。实测未缓存首播约 0.9 秒、缓存命中约 0.2 秒出画。
   - **观看历史页 (History)**：时间线记录、进度百分比、单条删除与清空二次确认。
   - **系统设置页 (Settings)**：清晰度偏好、连播倒计时滑块、画质增强偏好（当前仅原生）、缓存一键清理、匿名诊断日志导出。
...
│   │   ├── useEnhancementStore.tsx # 画质增强状态（后端当前仅提供原生档）
